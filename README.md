...
| **UniswapX** | DEX / intents | *candidate* / 2 attested | `uniswapx_order_hash` — the EIP-712 hash of an explicit Dutch order is **deterministic** (recomputable *in principle*; recipe on the roadmap). `uniswapx_build_order` builds a live signable intent. Non-custodial: the user signs; fillers execute. |
...
- **Candidate (roadmap):** UniswapX's **`uniswapx_order_hash`** — a UniswapX Dutch order is a signed EIP-712 *intent* whose order hash is a deterministic function of the explicit order struct, so it's recomputable *in principle* (anyone recomputes it byte-for-byte before a filler fills it, and the signature recovers to the swapper — same shape as our L4 check); the independent recipe (`uniswapx-order.v0`) is the next to ship. Everything else with no recipe now What stays Attested (LI.FI routes, Flashbots submits, Alchemy reads) is Attested *by nature*: its output depends on live state no one can reproduce offline, so there's no derivation to recompute.
